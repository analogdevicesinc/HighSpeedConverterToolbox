    ...
    methods    
        function SaveRxBeam(obj, ChipIDIndx, ChIndx, State, Attn, Gain, Phase)
            ...
            obj.setAttributeRAW(sprintf('voltage%d', ChIndx), ...
                'beam_pos_save', sprintf('%d, %d, %d, %f', State, Attn, Gain, Phase), false, obj.ChipID{ChipIDIndx});
        end
        ...
        function SaveTxBeam(obj, ChipIDIndx, ChIndx, State, Attn, Gain, Phase)
             ...
             validateattributes( Gain, { 'double','single', 'uint32' }, ...
                ...
                '', 'Gain');             
            obj.setAttributeRAW(sprintf('voltage%d', ChIndx), ...
                'beam_pos_save', sprintf('%d, %d, %d, %f', State, Attn, Gain, Phase), true, obj.ChipID{ChipIDIndx});
        end
    end
...
%             obj.pNumBufferBytes = coder.const(obj.numBufferBytes);
        ...
        function setupInit(obj)
            ...
            obj.ElementR = zeros(size(obj.ArrayElementMap));
            obj.ElementC = zeros(size(obj.ArrayElementMap));
