...
REPLACEMENT = (
    ...
    "  foreach _adi_ev {ADI_MATLAB MATLAB SKIP_SYNTHESIS ADI_SKIP_SYNTHESIS} {\n"
    "    if {[info exists ::env($_adi_ev)]} {\n"
    "      dict set _adi_saved_matlab_env $_adi_ev $::env($_adi_ev)\n"
    "      unset ::env($_adi_ev)\n"
    "    }\n"
    "  }\n"
    "  set _adi_exec_rc [catch {eval exec $make_command} _adi_exec_result _adi_exec_options]\n"
    "  dict for {_adi_ev _adi_val} $_adi_saved_matlab_env {\n"
    "    set ::env($_adi_ev) $_adi_val\n"
    "  }\n"
    "  if {$_adi_exec_rc} {\n"
    "    cd $current_dir\n"
    "    return -options $_adi_exec_options $_adi_exec_result\n"
    "  }\n"
)
...
GLOB_FIX = (
    ...
    "      error \"Expected exactly one xcvr cfng file, found [llength $_adi_xcvr_cfng_glob]\"\n"
    "    }\n"
