...
% custom source files
% NOTE: quiet.mk and Makefile are HDL-repo-root files (siblings of the
% projects/library/scripts folders). Since hdl_2026_r1, transceiver-based
% reference designs call adi_xcvr_project, which shells out to `make` in
% projects/xcvr_wizard/<carrier>; that Makefile chain includes ../../../quiet.mk.
% HDL Coder only copies the folders listed here into the generated work area,
% so quiet.mk must be listed explicitly or the xcvr sub-build fails with
% "../../../quiet.mk: No such file or directory".
hRD.CustomFiles = {...
    fullfile('projects')...
    fullfile('library')...
    fullfile('scripts')...
