classdef DAQ3Tests < HardwareTests
...
%             plot(freqRangeRx, FFTRxData);
    ...
    methods (Test)
        ...
        function testDAQ3Rx(testCase)    
            % Test Rx DMA data output
            rx = adi.DAQ3.Rx('uri',testCase.uri);
...
%             freqEst2 = meanfreq(double(real(out(:,2))),rx.SamplingRate);
        ...
        function testDAQ3RxWithTxData(testCase)
...
%             plot(real(out));
        ...
        function testDAQ3RxWithTxDataTwoChan(testCase)
            % Test Tx DMA data output
